...
import random
# Note: Our change to their agent code is adding seeding to the agent so that we can reproduce results
seed = 42 
random.seed(seed)
np.random.seed(seed)

'''
Note: This code is from https://raw.githubusercontent.com/datamllab/rlcard/master/rlcard/agents/cfr_agent.py, an implementation of CFR from which we test our trained agents against.
'''
class CFRAgent():
